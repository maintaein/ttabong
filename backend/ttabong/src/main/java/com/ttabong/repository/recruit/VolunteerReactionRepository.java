...
public interface VolunteerReactionRepository extends JpaRepository<VolunteerReaction, Integer> {
    ...
    @Query("SELECT v FROM VolunteerReaction v WHERE v.volunteer.userId = :userId AND v.isLike = TRUE AND v.isDeleted = FALSE AND v.id > :cursor ORDER BY v.createdAt DESC")
    List<VolunteerReaction> findLikedTemplatesByUserId(Integer userId, Integer cursor, Integer limit);
